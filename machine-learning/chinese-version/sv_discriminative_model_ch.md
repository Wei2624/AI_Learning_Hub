...
其中j可以遍历特征向量中的所有维度。$\alpha$ 被称为**学习率(learning rate)**，它控制模型学习/训练的速度。这种一步步更新来找到最小值叫**迭代算法(iterative algorithm)**。它将产生良好的对全局最小值的近似。
...
# 3 概率解释
正则方程是一种直接地确定地找解的方法，让我们看看如何从概率的角度解释它。概率解释下最终应该得到相同的结果。
...
我们需要找到给定x时，使y的概率最大化的 $\theta$。 我们称之为**最大似然法**。 为简化，我们来找**最大对数似然log likelihood**：
...
# 5 分类与逻辑回归
我们可以将逻辑回归当做成一个特殊的回归问题，我们只回归到一组二元值0和1。有时，我们也使用-1和1来分类，我们分别称它为负类和正类。
...
输出范围从0到1。 这直观地解释了为什么我们将其称为回归，因为它在连续的空间中输出。 该值表示属于将输出回归为某一个特定类的概率， 所以它是一个分类器。
...
从第一行到第二行，我们使用上面导出的logistic函数的导数。 这为我们提供了特征向量上每个维度的更新方法。 虽然在这种情况下我们有与LMS相同的算法，但在这种情况下的假设是不同的。 当我们谈论广义线性化模型时，使用相同的等式并不奇怪。

# 6 跑个题：感知器学习算法
我们在之后的学习理论中会继续讨论，简单来讲，我们把模型假设假设调整为：
...
# 7 牛顿最大化方法
我们想要找到函数f的根，牛顿法允许我们以二次速度完成这项任务。 这个方法是：随机初始化 $x_0$ 并找到 $f(x_0)$ 的切线，标记为$f^{\prime}(x_0)$。 我们使用 $f^{\prime}(x_0)$ 的根作为新x。 我们将新x和旧x之间的距离定义为 $\Delta$ 。 下图可以展示这个过程：
...
在我们更新逻辑回归和最小均方回归时，它们形式一样，这看似巧合。因为它们是广义线性模型大家族中的特殊情况。 它被称为线性的原因是该族中的每个分布中的变量和它们的权重之间存在线性关系。

在进入广义线性模型之前，我们讨论指数族分布作为它的基础。 如果可以用下面的形式来表示一个分部，那么它就是指数族中的一类分布：
...
其他指数分布：多项分布，泊松分布，伽马分部和指数分布，贝塔分部和的狄利克雷分部。 由于它们都是指数族分部，我们可以用统一的形式来研究指数族，并通过改变 $\eta$ 以建立不同的模型。 

# 9 建立GLM
如上所述，一旦我们知道了T，a和b，就已经确定了一类分布。 我们只需要找到 $\eta$ 来明确具体的分布。

例如，假设我们想要基于给定的x预测y。 在继续推导出这个回归问题的GLM之前，我们对此做出三个主要假设：

**(1)**我们总是假设 $y \lvert x;\theta \thicksim \text{ExponentialFamily}(\eta)$. 
**(2)**通常，我们想要预测给定x的T（y）的期望值。 一般情况下，我们有 $T(y) = y$。 形式上，我们有 $h(x) = \mathbb{E}[y\lvert x]$，这对于逻辑回归和线性回归都是正确的。 注意，在逻辑回归中，我们总是有 $\mathbb{E}[y\lvert x] = p(y=1\lvert x;\theta)$
...
。 通常，响应函数是 $\eta$ 的函数，并给出 $\eta$ 和分布参数之间的关系，而链接函数产生 $\eta$ 作为分布参数的函数。 反函数意味着用一者来表达另一者。 从上面的推导，我们知道伯努利的典型响应函数是逻辑函数，而高斯函数的典型响应函数是均值函数。
...
（3），我们知道对于对于$i=1，2，\dots，k-1$， $\eta_i = \theta_i^Tx$ ，并且 $\theta_i \in \mathbb{R}^{n+1}$ 是我们GLM模型的参数。而 $\theta_k$ 只是0，因此 $\eta_k = 0$ 。 现在，我们有基于x的模型：
